  ...
  <v-list id="drawerNavigation" >
    ...
    <v-list-item-group v-model="activate" color="blue"  @change="activateChange">
      <div v-for="item in menu" :key="item['id']">
        <menu-item  v-if="!$notEmpty(item['children'])&&!item['hidden']" :icon="item['icon']" :value="item['id']" :itemTitle="item['title']" :link="$addBaseURL(item['path'],true)"></menu-item><!--没有children-->
        <nested-menu v-if="$notEmpty(item['children'])&&!item['hidden']" :menu="item" :key="item['id']" :pLink="$addBaseURL(item['path'],true)"></nested-menu>
      </div>
    </v-list-item-group>
  </v-list>
</template>
